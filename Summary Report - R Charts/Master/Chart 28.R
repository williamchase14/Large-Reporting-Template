...
build_slide <- function(
  ...
  data <- data %>%
    ...
    # decode multi-escaped ampersands in both fields
    mutate(
  ...
  # 4) Filter to selected pillars AND engagement type (Acceleration Grant)
  selected_pillars <- c(
  ...
  filtered_data <- latest_by_school %>%
    filter(
      engagement_type == "3. Diagnostic, Implementation, & Acceleration Grant",
