...
build_slide <- function(
  ...
  # 4) Academic Design & Support pillar by school
  academic_design_data <- latest_by_school %>%
    filter(Pillar == "Academic Design & Support") %>%
    group_by(School) %>%
    summarise(
  ...
  num_schools <- dplyr::n_distinct(academic_design_data$School)

  title_txt <- "Progress Score - Academic\nDesign & Support"
  subtitle_txt <- paste0("(", num_schools, " Institutions)")

  # 5) Plot (no file saving)
  plt <- ggplot(
    academic_design_data,
