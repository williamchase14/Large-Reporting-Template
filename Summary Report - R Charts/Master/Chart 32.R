...
build_slide <- function(
  ...
  # 2) Prep fields for latest-survey logic
  data <- data %>%
    mutate(
  ...
  # 4) Region-only filter and cleanup
  ...
  # 5) Group by Region: average score + distinct school count
  region_averages <- filtered_data %>%
    group_by(region) %>%
    summarise(
      Average_Weighted_Completed = mean(`Weighted % Complete`, na.rm = TRUE),
      Schools_in_Region = n_distinct(School),
      .groups = "drop"
    ) %>%
    mutate(Region_Label = paste0(region, " (", Schools_in_Region, ")")) %>%
    arrange(Average_Weighted_Completed)

  num_schools <- dplyr::n_distinct(filtered_data$School)

  # 6) Build horizontal bar chart
  title_txt <- "Progress Score Across Regions"
  subtitle_txt <- paste0("(", num_schools, " Institutions)")

  plt <- ggplot(
    region_averages,
    aes(x = reorder(Region_Label, Average_Weighted_Completed),
