...
build_slide <- function(
  ...
  # Clean the 'Weighted % Complete' column
  data <- data %>%
  ...
  # -----------------------------
  # Normalize pillar names (decode multi-escaped '&')
  # -----------------------------
  ...
  # Define the 7 pillars to include (canonical labels)
  selected_pillars <- c(
  ...
  # Filter to launch year and selected pillars
  filtered_data <- norm %>%
    filter(Pillar %in% selected_pillars, launch_year == 2022)
  ...
  # Highlight Top/Bottom N (per school across pillars)
  n_top <- 1
  n_bottom <- 1

  plot_df <- summary_with_total %>%
    group_by(School) %>%
    mutate(
      r_top = min_rank(desc(Average_Weighted_Complete)),
      r_bottom = min_rank(Average_Weighted_Complete),
      band = case_when(
        School == "<b>Grand Total</b>" ~ "Total",
        r_top <= n_top ~ "Top",
        r_bottom <= n_bottom ~ "Bottom",
        TRUE ~ "Other"
      ),
      fill_value = case_when(
        band == "Other" ~ NA_real_,
        TRUE ~ Average_Weighted_Complete
      )
    ) %>%
    ungroup()

  # Gridline positions
  nx <- n_distinct(plot_df$Pillar)
  ny <- n_distinct(plot_df$School)
  ...
  plt <- ggplot(plot_df, aes(x = Pillar, y = School)) +
    # white band behind Grand Total
    geom_tile(
      data = dplyr::filter(plot_df, School == "<b>Grand Total</b>"),
