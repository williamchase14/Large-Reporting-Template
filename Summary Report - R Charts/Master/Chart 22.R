...
build_slide <- function(
  ...
  data <- read_csv(source_file, show_col_types = FALSE) %>%
    filter(launch_year == 2023) %>%
    mutate(
      ret_2022 = as.numeric(ret_2022) / 100,
      ret_2023 = as.numeric(ret_2023) / 100
    )

  # 2) Average retention per School for 2022 and 2023
  ...
  # 3) Sort schools by 2023 retention (ascending) and drop all-missing rows
  ...
  # 4) Label placement rules (higher above, lower below; ties -> 2023 on top)
  tol <- 1e-9
  average_retention <- average_retention %>%
    ...
    ungroup() %>%
    mutate(Year = as.character(Year))  # for scale mapping

  label_nudge <- 0.025

  # 5) Plot
  title_txt <- "Average Retention for Launch Year 2023"
