...
build_slide <- function(
  ...
  # Filter to launch year 2023 + selected pillars
  filtered_data <- norm %>%
    filter(Pillar %in% selected_pillars, launch_year == 2023)
  ...
  # Highlight Top/Bottom N (kept for parity; not used for fill here)
  n_top <- 1
  n_bottom <- 1
