...
build_slide <- function(
  ...
  # 4) Career Oriented Learning pillar by school
  career_learning_data <- latest_by_school %>%
    filter(Pillar == "Career Oriented Learning") %>%
    group_by(School) %>%
    summarise(
  ...
  num_schools <- dplyr::n_distinct(career_learning_data$School)

  title_txt <- "Progress Score - Career-Oriented\nLearning"
  subtitle_txt <- paste0("(", num_schools, " Institutions)")

  # 5) Plot (no file saving)
  plt <- ggplot(career_learning_data,
