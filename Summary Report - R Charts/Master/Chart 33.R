...
data <- data %>%
  ...
  # 4) State-only filter and cleanup
  ...
  # 5) Group by State: average score + distinct school count
  state_averages <- filtered_data %>%
    group_by(state) %>%
    summarise(
      Average_Weighted_Completed = mean(`Weighted % Complete`, na.rm = TRUE),
      Schools_in_State = n_distinct(School),
      .groups = "drop"
    ) %>%
    mutate(State_Label = paste0(state, " (", Schools_in_State, ")")) %>%
    arrange(Average_Weighted_Completed)

  num_schools <- dplyr::n_distinct(filtered_data$School)

  # 6) Build horizontal bar chart
  title_txt <- "Progress Score Across States"
  subtitle_txt <- paste0("(", num_schools, " Institutions)")

  plt <- ggplot(
    state_averages,
    aes(x = reorder(State_Label, Average_Weighted_Completed),
    ...
    geom_text(aes(label = paste0(round(Average_Weighted_Completed * 100, 0), "%")),
              hjust = 1.1, size = 3, color = "#ffffff", fontface = "bold") +
