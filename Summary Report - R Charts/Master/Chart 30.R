...
build_slide <- function(
  ...
  data <- data %>%
    ...
    # Decode multi-escaped ampersands in engagement_type and Pillar
    mutate(
  ...
  # 4) Filter to selected pillars AND engagement type (Diagnostic & Implementation)
  selected_pillars <- c(
  ...
  filtered_data <- latest_by_school %>%
    filter(
      engagement_type %in% c("2. Diagnostic & Implementation", "2. Diagnostic &amp; Implementation"),
  ...
  # 6) Build chart
  title_txt <- "Diagnostic & Implementation – Average by Pillar"
