...
build_slide <- function(
  ...
  latest_by_school <- data %>%
    ...
    ungroup() %>%
    filter(!is.na(`Weighted % Complete`))  # keep as per original script

  # 4) Financial Wellness pillar by school
  financial_wellness_data <- latest_by_school %>%
    filter(Pillar == "Financial Wellness") %>%
    group_by(School) %>%
    summarise(
  ...
  num_schools <- dplyr::n_distinct(financial_wellness_data$School)

  title_txt <- "Progress Score - Financial\nWellness"
  subtitle_txt <- paste0("(", num_schools, " Institutions)")

  # 5) Plot (no file saving)
  plt <- ggplot(financial_wellness_data,
