...
build_slide <- function(
  ...
  # 2) Clean and prepare for latest-survey logic
  data <- data %>%
  ...
  # 4) Average per School + carry Institution Type (latest only)
  Progress_Ave <- latest_by_school %>%
    group_by(School) %>%
    summarise(
      Average_Weighted_Completed = mean(`Weighted % Complete`, na.rm = TRUE),
      institution_type = dplyr::coalesce(
        dplyr::first(institution_type[!is.na(institution_type)]),
        dplyr::first(institution_type)
      ),
      .groups = "drop"
    ) %>%
  ...
  # 5) Create the plot
  title_txt <- "Progress Score by Institution Type"
  ...
  plt <- ggplot(
    ...
    aes(
      ...
      fill = as.factor(institution_type)
    )
  ) +
    ...
    scale_fill_manual(
      values = c("2-year" = "#E03244", "4-year" = "#0554A3", "Other" = "#2B3555"),
      name = element_blank()
    ) +
    labs(x = NULL, y = NULL) +
    theme_classic() +
    theme(
      ...
      legend.title = element_text(color = "#2B3555"),
